...
with tab1:
   ...
   # --- POSITION-SPECIFIC SCOUTING REPORT ---
    def generate_scout_report(player, df_context):
        ...
        # Keys: 'primary' (must be high for archetype), 'bonus' (extra traits to mention)
        pos_logic = {
            'PG': {'primary': ['PAS', 'HDL'], 'bonus': ['STL', 'IQ'], 'min': 70},
            'SG': {'primary': ['SCR', 'HDL'], 'bonus': ['STL',], 'min': 65},
            'SF': {'primary': ['SCR', 'DEF'], 'bonus': ['ORB', 'DRB'], 'min': 65},
        ...
        # 2. Identify High Traits (Relative to class average)
        high_traits = [stat for stat in ['SCR', 'DEF', 'PAS', 'IQ', 'BLK', 'STL', 'ORB', 'HDL'] 
                       if stat in player and player[stat] > avg[stat] + 8]
        
        # 3. Dynamic Archetype Logic
        archetype = "Well-Rounded Prospect"
        
        # Check primary skills for position
        ...
        if pos == 'PG' and primaries_met:
            ...
            archetype = "Sharpshooting Guard" if player.get('FG_ATB', 0) > 38 else "Scoring Specialist"
            ...
            archetype = "Paint Protector" if player.get('BLK', 0) > 60 else "Glass Cleaner"
        ...
        if player.get('DriveShot', 0) > 8: habits.append("driving the lane attempting to finish")
        if player.get('PASS', 0) > 80: habits.append("passing instead of taking the shot")
        ...
        report = (
            ...
            f"While he is currently ranked **#{int(get_ranks('Overall_Pot')[0])}** among {pos}s in potential, "
